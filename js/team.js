    ...
    var selection = d3.select("#teamChart");

    var margin_team = {top: 30, right: 10, bottom: 10, left: 10},

    width_team = selection[0][0].clientWidth,
...
var vis = d3.select("#teamChart").append("svg")
    .attr("width", width_team + margin_team.right + margin_team.left)
    .attr("height", height_team + margin_team.top + margin_team.bottom)
  .append("g")
    .attr("transform", "translate(" + margin_team.left + "," + margin_team.top + ")");

d3.json("data/bracket.json", function(json) {
  root = json;
  root.x0 = height_team / 2;
  root.y0 = width_team / 2;
  
  var t1 = d3.layout.tree().size([height_team, halfWidth]).children(function(d){return d.winners;}),
      t2 = d3.layout.tree().size([height_team, halfWidth]).children(function(d){return d.challengers;});
  t1.nodes(root);
  t2.nodes(root);
